(() => {
  ...
  function radiusForScore(score) {
    // Higher score => closer to center. Keep a minimum radius so nothing sits on top of the sweep origin.
    const maxR = 250;
    const minR = 40;
  ...
  function radiusForBlip(score) {
    const minSize = 5;
    const maxSize = 15;
    return minSize + (Math.max(0, Math.min(100, score)) / 100) * (maxSize - minSize);
  }
  ...
  function buildRadarStatic() {
    ...
    [250, 187, 125, 62].forEach((r, i) => {
      ...
      c.setAttribute("stroke-width", i === 0 ? 1.4 : 1);
      radarSvg.appendChild(c);
    });

    // cross-hairs
    [[cx - 250, cy, cx + 250, cy], [cx, cy - 250, cx, cy + 250]].forEach(([x1, y1, x2, y2]) => {
      const l = document.createElementNS(NS, "line");
      l.setAttribute("x1", x1); l.setAttribute("y1", y1);
      l.setAttribute("x2", x2); l.setAttribute("y2", y2);
      l.setAttribute("stroke", "var(--grid-soft)");
    ...
    stop1.setAttribute("stop-color", "var(--sweep)");
    stop1.setAttribute("stop-opacity", "0");
    const stop2 = document.createElementNS(NS, "stop");
    stop2.setAttribute("offset", "100%");
    stop2.setAttribute("stop-color", "var(--sweep)");
    stop2.setAttribute("stop-opacity", "0.28");
    ...
    const x2 = cx + 250 * Math.cos(-Math.PI / 2);
    const y2 = cy + 250 * Math.sin(-Math.PI / 2);
    const x3 = cx + 250 * Math.cos(-Math.PI / 2 + wedgeAngle);
    const y3 = cy + 250 * Math.sin(-Math.PI / 2 + wedgeAngle);
    ...
    const blipLayer = document.createElementNS(NS, "g");
  ...
  function renderRadar() {
    ...
    for (const st of items.values()) {
      ...
      const r = radiusForScore(st.score);
      ...
      if (st.band !== "NORMAL") {
        ...
        ring.setAttribute("r", radiusForBlip(st.score) + 6);
      ...
      core.setAttribute("r", radiusForBlip(st.score));
      ...
      label.setAttribute("y", y - radiusForBlip(st.score) - 6);
  ...
  function renderList() {
    listBody.innerHTML = "";
    const sorted = [...items.values()].sort((a, b) => b.score - a.score);
    for (const st of sorted) {
      const tr = document.createElement("tr");
      tr.innerHTML = `
        <td style="font-family:var(--mono); font-weight:600;">${st.symbol}</td>
        <td>${st.sector}</td>
        <td class="num">${st.price.toFixed(2)}</td>
        <td class="num">${st.changePct !== undefined ? st.changePct.toFixed(2) + "%" : "&mdash;"}</td>
        <td class="num">${st.score}</td>
        <td><span class="status-pill pill-${st.band}">${BAND_LABEL[st.band]}</span></td>
      `;
  ...
  function renderChips() {
    ...
    for (const st of items.values()) {
      ...
      li.innerHTML = `${st.symbol} <button class="remove-btn" title="Remove">&times;</button>`;
      li.querySelector(".remove-btn").addEventListener("click", async (e) => {
        e.stopPropagation();
        await api(`/api/watchlist/${USER_ID}/${st.symbol}`, { method: "DELETE" });
  ...
  function renderAll() {
    if (currentView === "radar") renderRadar(); else renderList();
    renderChips();
  }
  ...
  async function showExplain(symbol) {
    ...
    const data = await api(`/api/explain/${symbol}`);
    document.getElementById("explainEmpty").classList.add("hidden");
    const content = document.getElementById("explainContent");
    content.classList.remove("hidden");
    document.getElementById("explainSymbol").textContent = data.symbol;
    document.getElementById("explainScore").textContent = data.score;
    document.getElementById("explainBand").textContent = BAND_LABEL[data.band] || data.band;
    ...
    for (const [key, val] of Object.entries(data.components)) {
      ...
      chip.textContent = `${labels[key] || key} +${val}`;
  ...
  async function loadDigest() {
    const data = await api(`/api/digest/${USER_ID}`);
    const banner = document.getElementById("digestBanner");
    const calm = document.getElementById("calmBanner");
    if (data.count > 0) {
      document.getElementById("digestCount").textContent = data.count;
      document.getElementById("digestText").textContent =
        data.count === 1
      ...
      for (const ch of data.changes.slice(0, 5)) {
        ...
        div.innerHTML = `<strong>${ch.symbol}</strong> &mdash; ${ch.narrative}`;
      ...
      document.getElementById("calmText").textContent =
        `${count} stocks reviewed \u2014 nothing needs you right now.`;
      calm.classList.remove("hidden");
    }
  }

  document.getElementById("digestDismiss").addEventListener("click", async () => {
    await api(`/api/checkin/${USER_ID}`, { method: "POST" });
    document.getElementById("digestBanner").classList.add("hidden");
    loadDigest();
  });

  // ---------------- Watchlist load / add ----------------
  async function loadWatchlist() {
    const data = await api(`/api/watchlist/${USER_ID}`);
    items.clear();
  ...
  async function addSymbol() {
    ...
    if (!symbol) return;
    try {
      await api(`/api/watchlist/${USER_ID}`, {
        method: "POST",
  ...
  document.querySelectorAll(".toggle-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      document.getElementById("radarView").classList.toggle("hidden", currentView !== "radar");
  ...
  document.getElementById("shockBtn").addEventListener("click", async () => {
    ...
    await api("/api/simulate/shock", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ symbol, direction, magnitude: 0.055, volumeMultiplier: 7 }),
    });
  });
  ...
  function connectStream() {
    ...
    es.onerror = () => {
      ...
      connStatus.innerHTML = '<span class="dot"></span> reconnecting&hellip;';
    };
    es.onmessage = (evt) => {
      ...
      if (touchedWatchlist) {
        ...
        if (selectedSymbol && items.has(selectedSymbol)) {
          // keep explain panel fresh if the selected symbol just ticked
